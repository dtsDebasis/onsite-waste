...
class KnowledgeContentTag extends Model
{
    use HasFactory;

    protected $table = 'knowledge_content_tags';
    ...
    public function getListing($srch_params = [], $offset = ''){
        ...
        $listing = self::select($select)->with($with_det)
            ...
            ->when(isset($srch_params['tag']), function($q) use($srch_params){
                return $q->where($this->table.".tag", "LIKE", "%{$srch_params['tag']}%");
            })
