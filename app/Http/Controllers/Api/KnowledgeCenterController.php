...
class KnowledgeCenterController extends Controller
{
    ...
    public function getKnowledgeContent(Request $request)
    {
        try {
            ...
            $input['sateIds'] = $sateIds;
            $input['specialityArray'] = $specialityArray;
            $input['packageArray'] = $packageArray;
            $with = ['tags', 'kcstates', 'kcspecialities'];
            if (isset($input['content_id'])) {
                $data = $modelObj->getListing([
                    'id' => $input['content_id'],
                    'sateIds' => $input['sateIds'],
                    'specialityArray' => $input['specialityArray'],
                    'packageArray' => $input['packageArray'],
                    'with' => $with
                ]);
            } else {
                $kw_category_id = 0;
    ...
    public function getContentByWizardCat(Request $request)
    {
        try {
            ...
            if ($validator->fails()) {
                ...
                $param['sateIds'] = $sateIds;
                $param['specialityArray'] = $specialityArray;
                $param['packageArray'] = $packageArray;

                $wizard = app('App\Models\KnowledgeWizard')->getListing(['id' => $input['id']]);
                if ($wizard) {
                    $data = app('App\Models\KnowledgeCategory')->getListing(['id' => $wizard->category_id, 'with' => [
                        'knowledge_content' => function ($q) use($param) {
                            return $q->with(['kcstates', 'kcspecialities'])->where('status', 1)
                            ->whereIn('service_type', $param['packageArray'])
                            ->whereHas('kcspecialities', function ($query) use ($param) {
                                $query->whereIn('speciality_id',$param['specialityArray']);
                            })
                            ->whereHas('kcstates', function ($query) use ($param) {
                                $query->whereIn('state_id',$param['sateIds']);
                            })
                            ->orderBy('rank', 'ASC');
                        }
                    ]]);
    ...
    public function getContentWithCat(Request $request)
    {
        try {
            ...
            if (isset($input['category_id']) && $input['category_id']) {
                $data = app('App\Models\KnowledgeCategory')->getListing(['id' => $input['category_id'], 'status' => 1, 'with' => ['knowledge_content' => function ($q) {
                    return $q->orderBy('rank', 'ASC');
                }]]);
            } else {
                //,'with'=>['knowledge_content'=>function($q){return $q->orderBy('rank','ASC')->limit(5);}]
                $data = app('App\Models\KnowledgeCategory')->getListing(['kw_category_id' => 0, 'status' => 1]);
