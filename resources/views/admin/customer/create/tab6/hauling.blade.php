...
<div class="tab-pane active" id="settings-1" role="tabpanel">
    <div class="card">
        <div class="card-body mb-4">
            ...
            @else
                {!! Form::open(array('route' => ['customers.hauling-store-upadte',$company->id],'method'=>'POST', 'enctype'=>'multipart/form-data','id'=>'branch-hauling-form','class' => 'form-horizontal')) !!}
            @endif
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            @php($branch_id = Request::old('branch_id')?Request::old('branch_id'):(Request::get('branch_id')?Request::get('branch_id'):(($hauling && isset($hauling->branch_id))?$hauling->branch_id:null)))
                                    ...
                                    @if(count($hauling_list))
                                        @foreach($hauling_list as $hkey => $hval)
                                            <tr class="">
                                                ...
                                                <td><a href="javascript:;" data-id="{{$hval->id}}" data-status="{{$hval->status}}" data-driver_name="{{$hval->driver_name}}" class="change-status"><span class="badge badge-pill badge-soft-success">{{isset($status_arr[$hval->status]) ? $status_arr[$hval->status]: 'NA'}}</span></a></td>
                                                <td>
                                                    ...
                                                    <a href="{{route('customers.create.hauling',[$id,'hid' => $hval->id])}}" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit" type="button" class="btn btn-primary btn-sm btn-rounded waves-effect waves-light">
